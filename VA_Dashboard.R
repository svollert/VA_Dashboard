...
ui = bs4DashPage(
  ...
  body = bs4DashBody(bs4TabItems(bs4TabItem(tabName = "dashboard1",
                                 ...
                                 bs4TabItem(tabName = "information",
                                            ...
                                            fluidRow(bs4Card(title = "Per-model metrics plot", "This set of plots gives an overview of generally good or weak models, hence serving as a starting point to detect potential model candidates for further re???nement or for the exclusion from the training process. The per-model metrics can be viewed at di???erent levels of granularity with the following subplots: a list of ranked and grouped models, a line plot with the model accuracies, and a box plot with the dispersions of recall, precision and F1-score.
                                                             In the model rank subplot, the models are ranked and grouped into strong, medium and weak models. For the ranking of the models, the underlying assumption is, that a good model has a high accuracy and a low classi???cation imbalance, i.e. all classes have a similarly high detection rate. In the second subplot the models' accuracies are contrasted with each other in order to a allow a coarse comparison of the models. In addition the accuracies are shown in reference to a virtual random classi???er, randomly classifying each instance with equal probability for each class, and to a baseline classi???er, assigning all instances to the majority class.
                                                             In the third subplot, the models are contrasted using box plots showing the dispersions of the per-class metrics. Box plots positioned at the top indicate stronger models while the height indicates a model's variability over the classes.", width = 6, height = 260),
                                                     bs4Card(title = "Model similarity plot", "The (dis-)similarities between models are visible in this plot. For each model the overall accuracy and the standard deviation of the true prediction rates are extracted, allowing to present all models in a 2D-scatter plot. Similar models are thereby placed close to each other, where multiple similar models might reveal clusters. Weak, strong, or models with highly di???erent results become obvious as outliers.", width = 6, height = 260)),
